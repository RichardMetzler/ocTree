...
IF(FREEIMAGE_ROOT)
  get_filename_component(FREEIMAGE_ROOT "${FREEIMAGE_ROOT}" ABSOLUTE)
  SET(SEARCH_PATHS
    ${FREEIMAGE_ROOT}
    ${FREEIMAGE_ROOT}/include
	${FREEIMAGE_ROOT}/include/FreeImage
    ${SEARCH_PATHS}
    )
ENDIF()
...
IF(NOT FREEIMAGE_LIBRARY)  # now look in system locations
  FIND_LIBRARY(FREEIMAGE_LIBRARY NAMES freeimage FreeImage)# HINTS /usr/lib/x86_64-linux-gnu)
ENDIF(NOT FREEIMAGE_LIBRARY)

SET(FREEIMAGE_LIBRARIES)
IF(FREEIMAGE_LIBRARY_RELEASE)
  SET(FREEIMAGE_LIBRARIES "optimized;${FREEIMAGE_LIBRARY_RELEASE}")
ENDIF()
IF(FREEIMAGE_PLUS_LIBRARY_RELEASE)
  SET(FREEIMAGE_LIBRARIES "${FREEIMAGE_LIBRARIES};${FREEIMAGE_PLUS_LIBRARY_RELEASE}")
ENDIF()
IF(FREEIMAGE_LIBRARY_DEBUG)
  SET(FREEIMAGE_LIBRARIES "${FREEIMAGE_LIBRARIES};debug;${FREEIMAGE_LIBRARY_DEBUG}")
ENDIF()

IF(FREEIMAGE_PLUS_LIBRARY_DEBUG)
  SET(FREEIMAGE_LIBRARIES "${FREEIMAGE_LIBRARIES};debug;${FREEIMAGE_LIBRARY_DEBUG}")
ENDIF()
