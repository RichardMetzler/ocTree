...
ocTree* ocTree::addVerticesToSelectionByCoordinates(float x, float y, float z, float radius, std::set<size_t> &intermediateSelection, bool debugInfo) {
	if (debugInfo) {
	...
	if (this->getisLeaf() == true) {
		...
		if (radius != 0) {
			...
			if (x - radius < m_minX) {
				...
				ocTree * neighbor = m_root->addVerticesToSelectionByCoordinates(x-radius,y,z,0,intermediateSelection,debugInfo);
			...
			if (y - radius < m_minY) {
				...
				ocTree * neighbor = m_root->addVerticesToSelectionByCoordinates(x,y-radius,z,0,intermediateSelection,debugInfo);
				verticesToCheck.insert(verticesToCheck.end(), neighbor->m_verticesInBounds.begin(), neighbor->m_verticesInBounds.end() );
				if (debugInfo) {
					std::cout << "neighbor LEVEL: " << neighbor->m_level << std::endl;
					std::cout << "verticesToCheck size (with neighbors): " << verticesToCheck.size() << std::endl;
			...
			if (z - radius < m_minZ) {
				...
				ocTree * neighbor = m_root->addVerticesToSelectionByCoordinates(x,y,z-radius,0,intermediateSelection,debugInfo);
				verticesToCheck.insert(verticesToCheck.end(), neighbor->m_verticesInBounds.begin(), neighbor->m_verticesInBounds.end() );
				if (debugInfo) {
					std::cout << "neighbor LEVEL: " << neighbor->m_level << std::endl;
					std::cout << "verticesToCheck size (with neighbors): " << verticesToCheck.size() << std::endl;
	...
	if (x < m_meanX) {
		if (y < m_meanY) {
			if (z < m_meanZ) {
				result = m_myChildren[6]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 110
			} else {
				result = m_myChildren[2]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 010
			}
		} else {
			if (z < m_meanZ) {
				result = m_myChildren[4]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 100
			} else {
				result = m_myChildren[0]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 000
			}
		}
	} else {
		if (y < m_meanY) {
			if (z < m_meanZ) {
				result = m_myChildren[7]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 111
			} else {
				result = m_myChildren[3]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection,debugInfo);	// 011
			}
		} else {
			if (z < m_meanZ) {
				result = m_myChildren[5]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 101
			} else {
				result = m_myChildren[1]->addVerticesToSelectionByCoordinates(x,y,z, radius, intermediateSelection, debugInfo);	// 001
			}
		}
	}
	return result;
...
ocTree* ocTree::removeVerticesFromSelectionByCoordinates(float x, float y, float z, float radius, std::set<size_t> &intermediateSelection, bool debugInfo) {
	if (debugInfo) {
		std::cout << "getNodeByCoordinates() called" << std::endl;
		std::cout << "Coordinates passed: x, y & z: " << x << ", " << y <<", " << z << std::endl;
	}
