...
//    	std::cout << "has colors"  << std::endl;
    	...
    	glEnableVertexAttribArray(1);
    	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, (void*)0);
    	// generieren; binden, enablen und vertexattribpointer;	NICHT bufferData
    	// das selbe für normals & indices(?)
    }
...
void Mesh::changeColors (std::set <size_t> &selectionIndices, float color[4])
{
//	std::cout << "changeColors called; length of selection: " << selectionIndices.size() << std::endl;
	...
	for (int n=0; n != selectionIndices.size(); ++n) {
		...
		if (m_colors[*it][3] == 0.99) {
			break;
		} else {
//			std::cout << "old R-value (should be 0.89): " << m_colors[*it][0] << std::endl;
			m_colors[*it][0] = color[0];
//			std::cout << "new R-value (should be 1.00): " << m_colors[*it][0] << std::endl;
...
{
	std::cout << "changeColor() called" << std::endl;
...
void Mesh::TESTchangeAllColors (void)
{
	std::cout << "TESTchangeAllColors() called" << std::endl;
...
void Mesh::TESTdebugColorOfVertex (size_t index)
{
	std::cout << "colors of vertex " << index << ": " << m_colors[index][0] <<", "<< m_colors[index][1] <<", "<< m_colors[index][2] << std::endl;
}
