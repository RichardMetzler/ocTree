...
\begin{description}
	...
	\item[Vertex Shaders] are small pieces of C-like code which can perform fast, basic operations on every vertex of a vertex data input stream. They are fed vertex attribute data, as specified in a VAO by the call of a openGL draw function. Usually, the sole purpose of the vertex shader is to perform perspective projection of 3D points (vertices) to their on-screen position in relation to a cemara within the scene. In other words, an incoming vertex $v$ is multiplied by a so-called \textit{model-view-perspective matrix}, resulting in a transformed vertex $v^[}'$.
