...
// void Object::draw(glm::mat4 proj_matrix, glm::mat4 view_matrix, glm::mat4 model_matrix)
void Object::drawWithoutParams()
{
//    glPushMatrix();
//    glMultMatrixf(glm::value_ptr(m_ObjectTrafo));
//    proj_matrix*view_matrix * m_ObjectTrafo
...
void Object::uploadAllMeshes()
{
    for(std::vector<Mesh*>::iterator it = m_Meshes.begin(); it != m_Meshes.end(); ++it){
        ...
        if ((*it)->get_m_vertices().size() == 0) {
        	// error
        	std::cout << "error reading mesh->m_vertices" << std::endl;
        } else {
        	// no error
        	std::vector<glm::vec3> verticesArray = (*it)->get_m_vertices();
        	for (int i = 0; i != verticesArray.size(); i++ ) {
        		// std::cout << i << ".x: " << verticesArray[i].x << ", " << i << ".y: " << verticesArray[i].y << ", " << i << ".z: " << verticesArray[i].z << ", "<< std::endl;
        	}
        	// std::cout << (*it)->get_m_vertices().size() << std::endl;
        }
    }
}
