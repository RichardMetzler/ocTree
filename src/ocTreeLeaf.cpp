...
std::list<glm::vec3> ocTreeLeaf::convertVectorToList(std::vector<glm::vec3> inputVector) {
	if (m_debugInfo) {
	...
	if (this->getisLeaf() == true) {
		...
		if (radius != 0) {
			...
			if (x - radius < m_minX) {
				...
				verticesToCheck.splice(verticesToCheck.end(), convertVectorToList(neighbor->m_verticesInBounds));
				if (debugInfo) {
